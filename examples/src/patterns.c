...
{
    const char *window_title = "SoRDIC Sine Pattern example";
    EVENT *event = NULL;
    ...
    SINE_PATTERN sp1, sp2;
    sine_pattern_make_tables(&sp1, 284, 300, 672, 200, 199);
    sine_pattern_make_tables(&sp2, 411, 220, 541, 350, 409);
    ...
    while (!quit_flag) {
        ...
        sine_pattern_render(&sp2, display_buffer(), xo1, yo1);
        sine_pattern_render(&sp1, srbuf, xo2, yo2);
        /** Add both buffers. */
        render_buffer_add_saturation(display_buffer(), srbuf);
        /** Display the result */
        display_show(0);
        fcnt++;
    ...
    sine_pattern_free(&sp1);
    sine_pattern_free(&sp2);
    engine_cleanup();
    return 0;
}